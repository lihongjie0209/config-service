	...
	r.wg.Go(func() {
		...
		for {
			if deleted, runErr := cleaner.RunOnce(runCtx); runErr != nil && !errors.Is(runErr, context.Canceled) {
				r.logger.ErrorContext(runCtx, "clean published config outbox events", "error", runErr)
			} else if deleted > 0 {
				r.logger.InfoContext(runCtx, "published config outbox events cleaned", "deleted", deleted)
			}
			select {
			case <-runCtx.Done():
				return

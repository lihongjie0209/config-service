...
const columns = `id, environment, tenant_id, service, config_key, config_value, secret_ref, status, revision, rollout_percentage, version, created_at, updated_at, created_by, updated_by`
...
func (r *SQLRepository) Insert(ctx context.Context, e sqlx.ExtContext, v Entry) error {
	_, err := e.ExecContext(ctx, r.db.Rebind(`INSERT INTO config_entries (`+columns+`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`), v.ID, v.Environment, v.TenantID, v.Service, v.Key, v.Value, v.SecretRef, v.Status, v.Revision, v.RolloutPercentage, v.Version, v.CreatedAt, v.UpdatedAt, v.CreatedBy, v.UpdatedBy)
	return err
}
func (r *SQLRepository) Update(ctx context.Context, e sqlx.ExtContext, v Entry, expected int64) error {
	result, err := e.ExecContext(ctx, r.db.Rebind(`UPDATE config_entries SET config_value=?,secret_ref=?,status=?,revision=?,rollout_percentage=?,version=version+1,updated_at=?,updated_by=? WHERE id=? AND version=?`), v.Value, v.SecretRef, v.Status, v.Revision, v.RolloutPercentage, v.UpdatedAt, v.UpdatedBy, v.ID, expected)
	if err != nil {
		return err
	}
...
func (r *SQLRepository) Resolve(ctx context.Context, environment, tenantID, service string, keys []string) ([]Entry, error) {
	...
	query, args, err := sqlx.In(`SELECT `+columns+` FROM config_entries WHERE environment=? AND tenant_id IN (?, '') AND service=? AND status='published' AND config_key IN (?) ORDER BY CASE WHEN tenant_id=? THEN 0 ELSE 1 END`, environment, tenantID, service, keys, tenantID)
	if err != nil {
		return nil, err
	}
	var values []Entry

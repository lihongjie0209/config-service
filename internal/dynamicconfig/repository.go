...
type Repository interface {
	...
	GetByScope(context.Context, string, string, string, string, string) (Entry, error)
	...
	Resolve(context.Context, string, string, string, string, []string) ([]Entry, error)
	List(context.Context, string, string, string, string, int, int) ([]Entry, int64, error)
...
const columns = `id, environment, tenant_id, application_id, service, config_key, config_value, secret_ref, status, revision, rollout_percentage, published_revision, review_comment, reviewed_by, reviewed_at, version, created_at, updated_at, created_by, updated_by`
...
func (r *SQLRepository) GetByScope(ctx context.Context, environment, tenantID, applicationID, service, key string) (Entry, error) {
	var v Entry
	err := r.db.GetContext(ctx, &v, r.db.Rebind(`SELECT `+columns+` FROM config_entries WHERE environment=? AND tenant_id=? AND application_id=? AND service=? AND config_key=?`), environment, tenantID, applicationID, service, key)
	if errors.Is(err, sql.ErrNoRows) {
		err = ErrNotFound
	}
	return v, err
...
func (r *SQLRepository) GetRevision(ctx context.Context, id string, revision int64) (Entry, error) {
	var v Entry
	err := r.db.GetContext(ctx, &v, r.db.Rebind(`SELECT e.id,e.environment,e.tenant_id,e.application_id,e.service,e.config_key,r.config_value,r.secret_ref,r.status,r.revision,r.rollout_percentage,e.published_revision,r.review_comment,r.reviewed_by,r.reviewed_at,r.version,r.created_at,r.updated_at,r.created_by,r.updated_by FROM config_revisions r JOIN config_entries e ON e.id=r.entry_id WHERE r.entry_id=? AND r.revision=?`), id, revision)
	if errors.Is(err, sql.ErrNoRows) {
		err = ErrNotFound
	}
	return v, err
}
func (r *SQLRepository) Resolve(ctx context.Context, environment, tenantID, applicationID, service string, keys []string) ([]Entry, error) {
	if len(keys) == 0 {
		return []Entry{}, nil
	}
	query, args, err := sqlx.In(`SELECT e.id,e.environment,e.tenant_id,e.application_id,e.service,e.config_key,r.config_value,r.secret_ref,'published' AS status,r.revision,r.rollout_percentage,e.published_revision,r.review_comment,r.reviewed_by,r.reviewed_at,e.version,e.created_at,e.updated_at,e.created_by,e.updated_by FROM config_entries e JOIN config_revisions r ON r.entry_id=e.id AND r.revision=e.published_revision WHERE e.environment=? AND ((e.tenant_id=? AND e.application_id=?) OR (e.tenant_id=? AND e.application_id='') OR (e.tenant_id='' AND e.application_id='')) AND e.service=? AND e.published_revision>0 AND e.config_key IN (?) ORDER BY CASE WHEN e.tenant_id=? AND e.application_id=? THEN 0 WHEN e.tenant_id=? AND e.application_id='' THEN 1 ELSE 2 END`, environment, tenantID, applicationID, tenantID, service, keys, tenantID, applicationID, tenantID)
	if err != nil {
		return nil, err
	}
	var values []Entry
...
func (r *SQLRepository) List(ctx context.Context, environment, tenantID, applicationID, service string, limit, offset int) ([]Entry, int64, error) {
	args := []any{environment, tenantID, applicationID, service}
	where := ` WHERE environment=? AND tenant_id=? AND application_id=? AND service=?`
	var total int64
